...
# 一般線形化モデル
*一般には「残差が任意の分布に従うと仮定したモデル」だが
本書ではロジスティック関数のみ言及*

![bg right:25% brightness:105%](img/hiei.webp)
...
> (交差エントロピーは) ベルヌーイ分布を誤差関数とした負の対数尤度関数
...
&= \prod^N_{i=1}\exp(t_i \log f_\theta(x_i) + (1-t_i) \log (1-f_\theta(x_i))) \\
&= e^{-\varepsilon_D(a, b)}
\end{aligned}
$$
